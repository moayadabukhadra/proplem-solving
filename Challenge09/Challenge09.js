...
// Resource:
// forEach: ://developer.mozilla.org/en-US/docs/Web/JavaScript/Referenhttpsce/Global_Objects/Array/forEach
...
let result=[];
    arr.forEach(element => {
  let newElement=  Math.round(element); 
...
const employeesBonus = (arr) => {
  ...
  arr.forEach(element => {
      ...
      if(element.workHours>=8){
       return element.salary + "100$"
      }
      else if(element.workHours<8){
      return  element.salary+"50$"
      }
...
let answer = mouseArray.element+keyBoardArray.element
newArr.push(answer);
   return  newArr
